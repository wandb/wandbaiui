...
let weaveCallsMemoizerPromise: Promise<any> | null = null;

const getWeaveCallsMemoizer = () => {
  if (!weaveCallsMemoizerPromise) {
    weaveCallsMemoizerPromise = createIndexedDBMemoizer(
      WEAVE_CALLS_DB,
      WEAVE_CALLS_STORE
    );
  }
  return weaveCallsMemoizerPromise;
};
...
export async function memoizedWeaveCalls(
  ...
  const memoizer = await getWeaveCallsMemoizer();
  return memoizer(weaveCalls)(params) as Promise<CallSchema[]>;
}
